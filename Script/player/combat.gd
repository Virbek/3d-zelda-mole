...
func _check_hits(from_t: float, to_t: float) -> void:
	...
	# On ne teste que la portion réellement dans la fenêtre active
	var a: float = maxf(from_t, w.x)
	var b: float = minf(to_t, w.y)
	if b <= a:
		return
	...
	for side in sides:
		var prev: Vector3 = rig.get_attack_position(a, side)
		for i in range(1, sweep_substeps + 1):
			var t: float = lerp(a, b, float(i) / sweep_substeps)
			var cur: Vector3 = rig.get_attack_position(t, side)
			_sphere_at((prev + cur) * 0.5, hit_radius + prev.distance_to(cur) * 0.5)
			prev = cur
